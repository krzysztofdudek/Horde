...
- Before a ticket lands, a reviewer now reads its change once. The reviewer has no way to approve anything: it writes down what is wrong, or it writes nothing. A Critical or Important finding sends the ticket back to its worker before the landing checks run, and costs one round, like a failed check. A Minor finding stays in the ticket's notes and never sends it back. The landing checks run in full whatever the reviewer wrote, so a reviewer who skimmed costs you one extra agent and nothing more.
