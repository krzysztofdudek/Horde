...
- The review drill no longer reads a reviewer's approval, a leftover reading from the per-node reviewer seat removed before 6.0.0. A hand-logged "review: <node> approve by <who>" line now counts as no review at all, the same as if the line were never written; only a change request still counts.
