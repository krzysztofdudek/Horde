...
The skill's own model page (`skills/horde/reference/model.md`, **Runner** section) is the canonical statement: there are two runners, `session` (the default) and `external`, and they differ only in who starts the workers the loop hands out — never in what the loop itself does. Every agent the skill raises is a plain subagent, spawned by the director directly: the architect one-shot per graph ruling (a fresh one rebuilds its context from the files), a worker per ticket, and each one-shot. README's `## Requirements` section carries the user-facing setup step (Node.js, Yggdrasil); this note is the engineering pointer behind it. The constraint the skill's topology rests on: a subagent is reachable and resumable by the agent that spawned it and by nobody else, which is why the director holds all of them itself and nothing in the cast raises anything of its own. There are four roles now, a closed list (`worker`, `architect`, `legislate`, `retro`), plus the consultant `refine.mjs` spawns off disk; the worker's own brief is the only one that carries its own worktree and branch — every other role's brief reads the trunk checkout, read-only.
