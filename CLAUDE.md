...
Horde is the third layer of the Yggdrasil family. Yggdrasil holds the architecture graph and the rails; Grain mines the first graph for a repository that has none; Horde works that graph when the mission does not fit in one agent's context, turning the agent into a director that raises an architect and a worker per ticket, plus one-shot consultants and a legislator per territory and a retrospective at the end, holding all of them to the same rules — evidence over reports, ask rather than guess, plain language back to the user, architecture checked before it lands. The three add-ons — Ratatoskr, Urd, Researcher — attach to the agent, not to the graph.
...
The skill's own model page (`skills/horde/reference/model.md`, **Runner** section) is the canonical statement: Claude Code's [Agent Teams](https://code.claude.com/docs/en/agent-teams) (`CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`, off by default, experimental, interactive sessions only) is an optional runner, never a requirement. The topology works without it — the director spawns the architect as its own teammate and every worker as its own subagent directly, one per ticket, in the default session-driven runner. Turning Agent Teams on changes nothing about the loop's four steps (`tick.mjs`); it only lets one long-lived Sonnet teammate drive that same loop on its own schedule instead of your next turn. README's `## Requirements` section carries the user-facing setup step (Node.js, Yggdrasil — nothing about Agent Teams); this note is the engineering pointer behind it. The constraint the skill's topology rests on either way: a teammate can spawn subagents (and subagents can spawn their own) but never another teammate — teammates are created by the top-level session alone, and a subagent is resumable by its parent and by nobody else. There are four roles now, a closed list (`worker`, `architect`, `legislate`, `retro`), plus the consultant `refine.mjs` spawns off disk; the worker's own brief is the only one that carries its own worktree and branch — every other role's brief reads the trunk checkout, read-only.
