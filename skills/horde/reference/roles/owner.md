...
```
node ${CLAUDE_PLUGIN_ROOT:-.claude/skills/horde}/scripts/node.mjs show {{node}}       # boundary, the rules in force, ports, charter, log
...
it, within the boundary `node.mjs show` prints; nothing outside except the ports on your border.
...
- **The inside of the node.** Within the charter and the ports you decide how things are done. You
  do not ask the steward or the director about the inside; you write the decision down and go on.
- **The charter agrees with the graph.** When your node's mapping, boundary or ports change, the
  charter changes in the same graph commit: a charter that still says "not yet in
...
- **Every change that touches your node.** You are held to the **review** discipline, printed in full
  ...
  (`tk.mjs list --review-pending --node {{node}}`); you read the diff against the charter and the ports and
...
- **Proposals.** At staffing you propose the node's tickets (`tk.mjs new … --node {{node}} --class …
  ...
  cheapest that passes verification — and the ports your node needs from or offers to its
  neighbours (`node.mjs contract propose <node> <port> "<why>" --as <test> --by {{name}}`, which
  proposes adding a port or bumping its version). Each port names the test that IS its promise, never a
  prose alone.
- **Four fields on every proposal — this is where the plan is written.** Nobody plans the mission
  ...
  - `--consumes <node>/<port>@<version>` — the port this ticket needs, at the version it needs.
  ...
  - `--produces <node>/<port>@<version>` — the port this ticket delivers. Raising a version is a
