...
test('cost.mjs: report, limit-reached (cost.json is read-only from here — roster.mjs spawn writes it)', async (t) => {
  ...
  await t.test('report sums runs and weighted cost from cost.json', () => {
    writeCostRuns(dir, 'mission1', [
      ...
      { name: 'mission1-worker-trunk-2', role: 'worker', class: 'sonnet', ticket: '001', team: 'trunk', wave: '1', at: new Date().toISOString() },
      { name: 'mission1-owner-auth-1', role: 'owner', class: 'opus', ticket: '002', team: 'trunk', wave: '1', at: new Date().toISOString() },
    ]);
    const r = run('cost.mjs', ['report'], dir);
    assert.equal(r.code, 0);
  ...
  await t.test('report --wave scopes to one wave', () => {
    writeCostRuns(dir, 'mission1', [
      { name: 'a', role: 'worker', class: 'haiku', ticket: '003', team: 'trunk', wave: '1', at: new Date().toISOString() },
      { name: 'b', role: 'worker', class: 'haiku', ticket: '004', team: 'trunk', wave: '2', at: new Date().toISOString() },
    ]);
...
test('cost.mjs: limit-reached against the charter\'s Limit line', async (t) => {
  ...
  writeCostRuns(dir, 'mission1', [
    { name: 'a', role: 'owner', class: 'opus', ticket: null, team: 'trunk', wave: '1', at: new Date().toISOString() },
    { name: 'b', role: 'owner', class: 'opus', ticket: null, team: 'trunk', wave: '1', at: new Date().toISOString() },
    { name: 'c', role: 'owner', class: 'opus', ticket: null, team: 'trunk', wave: '1', at: new Date().toISOString() },
